      ...
      <div class="v-cal-header__actions">
        <div class="actions-left">
          <button
            ...
            @click="goToToday"
            :class="{ 'v-cal-button--is-active': activeDate && activeDate.isSame( today, 'day' )}"
          >{{ labels.today }}</button>
          <button
        ...
        <div class="actions-right">
          <button
            ...
            :class="{ 'v-cal-button--is-active': activeView === view }"
          >{{ labels[view] | capitalizeFirstLetter }}</button>
        </div>
      </div>
...
export default {
    ...
    openEventDialog(data) {
      if (!this.disableDialog) {
        ...
        if (data instanceof Date) {
          ...
          config.startTime =
            data.time !== null ? moment(data.time, "HH") : null;
          config.endTime =
            data.time !== null ? moment(data.time, "HH").add(1, "h") : null;
        }

        EventDialog.show(config, fields).$on("event-created", (event) => {
  ...
  watch: {
    ...
    activeView() {
      this.$emit("view-changed", this.activeView, this.activeDate.toDate());
    },
  },
  computed: {
    newEvents() {
